...
public final class wq implements mp {
    ...
    public final void d(Object obj, Map map) {
        ...
        sb2.append(" ms.");
        a0.k(sb2.toString());
        a0.k("loadJavascriptEngine > /jsLoaded handler: Trying to acquire lock");
        dr drVar = this.f12317x;
        synchronized (drVar.f5484c) {
            a0.k("loadJavascriptEngine > /jsLoaded handler: Lock acquired");
            ...
            if (((AtomicInteger) crVar.f3469c).get() != -1 && ((AtomicInteger) crVar.f3469c).get() != 1) {
                ...
                drVar.f5487g = crVar;
                a0.k("Successfully loaded JS Engine.");
                a0.k("loadJavascriptEngine > /jsLoaded handler: Lock released");
                return;
            }
            a0.k("loadJavascriptEngine > /jsLoaded handler: Lock released, the promise is already settled");
        }
    }
}
