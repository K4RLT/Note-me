...
public abstract class f0 {
    ...
    public static void b(int i, int i10) {
        ...
        if (i >= 0) {
            ...
            c10 = z.c("%s (%s) must be less than size (%s)", "index", Integer.valueOf(i), Integer.valueOf(i10));
        } else {
            c10 = z.c("%s (%s) must not be negative", "index", Integer.valueOf(i));
        }
    ...
    public static void e(int i, int i10, int i11) {
        ...
        if (i >= 0 && i <= i11) {
            if (i10 >= 0 && i10 <= i11) {
                f10 = z.c("end index (%s) must not be less than start index (%s)", Integer.valueOf(i10), Integer.valueOf(i));
            } else {
    ...
    public static String f(String str, int i, int i10) {
        if (i < 0) {
            return z.c("%s (%s) must not be negative", str, Integer.valueOf(i));
        }
        if (i10 >= 0) {
            return z.c("%s (%s) must not be greater than size (%s)", str, Integer.valueOf(i), Integer.valueOf(i10));
        }
        q.x.n(g3.a.g(i10, "negative size: "));
        return null;
    }
